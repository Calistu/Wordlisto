char entradas()
{
    ...
    fgets(pet, 20, stdin);
    limpacr(pet);
    printf("\nTYPE A OWNER PASSWORD PHONE NUMBER\nSHOULD FOLLOW THIS DEFAULT  XXXXXXXX  ");
    fgets(numero, 10, stdin);
    limpacr(numero);
    sprintf(pparte, "%c%c%c%c", numero[0], numero[1], numero[2], numero[3]);
    ...
    limpacr(numero2);
    sprintf(pparte2, "%c%c%c%c", numero2[0], numero2[1], numero2[2], numero2[3]);
    sprintf(sparte2, "%c%c%c%c", numero2[4], numero2[5], numero2[6], numero2[7]);
    sprintf(nnumero2, "9%s", numero2);
    printf("\nTYPE THE OWNER PASSWORD BITH DAY\nIN THIS DATE FORMAT XX-XX-XXXX: ");
    fgets(data, 12, stdin);
    limpacr(data);
    b = strlen(data);
    for (a = 0; a <= b; a++) {
